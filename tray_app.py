...
def main():
    ...
    # core импортируется здесь, напрямую (не через поток сервера) — гарантирует,
    # что настройки уже загружены (core.py: load_settings()/apply_settings() на
    # уровне модуля) к моменту, когда решаем, открывать ли вкладку.
    sys.path.insert(0, RESOURCE_DIR)
    import core

    # Авто-открытие вкладки подавляется по логике «ИЛИ» двумя независимыми
    # способами: аргументом --autostart (для ярлыка в автозагрузке Windows,
    # без настроек) и галочкой в настройках (действует при любом запуске).
    # Ни один из способов не должен перезаписывать другой — открываем только
    # если НИ аргумента, НИ галочки.
    no_browser = "--autostart" in sys.argv or core.get_settings().get("no_browser_on_start", False)
    auto_browser = not no_browser
